...
const releasePayload = {
  ...
  assets: [{
    ...
    size: 228589568,
    browser_download_url: 'https://github.com/JackLee992/frost-story/releases/download/v0.1.0/FrostStory-arm64.apk'
  }]
};
...
async function stubReleaseData(page) {
  ...
  await page.route('https://raw.githubusercontent.com/JackLee992/frost-story/main/distribution/content-manifest.json', route => route.fulfill({
    status: 200, contentType: 'application/json', body: JSON.stringify({ version: '0.1.0' })
  }));
}
...
for (const viewport of [{ width: 390, height: 844, name: 'mobile' }, { width: 1440, height: 1000, name: 'desktop' }]) {
  test(`下载页 ${viewport.name} 布局和动态版本`, async ({ browser }) => {
    ...
    await expect(download).toHaveAttribute('href', /releases\/download\/v0\.1\.0\/FrostStory-arm64\.apk$/);
    await expect(page.locator('#downloadMeta')).toContainText('218 MB');
    await expect(page.locator('#contentVersion')).toHaveText('v0.1.0');
