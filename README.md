...
原创竖屏 Merge-2 合并剧情游戏。当前 v0.3 包含 6 章共 24 个剧情重建节点，并加入连续电影对白、全对白离线配音、亲手复苏互动、可持久化装修选择、原创关键 CG、剧情化村民请求、紧凑任务架、6 条 8 级合成链，以及可拖动吸边的内测爽玩控制台。界面与剧情支持简中、英、日、韩四语，字幕语言和配音语言可分别选择。
...
- 序章会由啾可在剧情里询问玩家称呼；昵称只保存在本地存档，之后可在设置里修改。游戏语言与配音语言首次默认一致，也可独立切换。
...
- 首局预摆四次合并连锁；首次发现和 3/5 连“暖流”立即返还金币、经验或体力。
- 前两章主线奖励支持快速连续升到 Lv.10，Lv.10 之后切回长线成长曲线。
- 主线缺料时的“找材料”会显示来源生成器、完整合成链和基础数量，并直接照亮下一对可合并材料或精确定位商店条目。
- 委托与主线默认收进 38px 双入口任务架，只允许主动展开一栏；收起状态不会覆盖合成盘。
- 6 段分章 BGM、风雪环境声、9 个操作/剧情音效；序章、玩家档案、24 段主线、村民请求/答谢和啾可引导共 216 句，中英日韩四套离线角色配音共 864 条，并在对白时自动压低音乐。
- Android 全屏沉浸式显示，使用真机挖孔和系统手势边距约束核心 UI。

竞品机制证据、原创边界与逐项对齐结果见 [竞品拆解](docs/01-竞品拆解与技术路线.md)、[玩法对齐验收矩阵](docs/03-竞品玩法对齐验收.md)、[CG / 叙事交互升级报告](docs/06-竞品CG与叙事交互升级.md)、[前三章爽感与材料引导](docs/07-前三章爽感与材料引导.md) 和 [游戏设计书目与前三章二次迭代](docs/09-游戏设计书目与前三章二次迭代.md)。
...
多语言内容采用“本地模型初译 → ChatGPT 审校覆盖层 → 自动门禁”的可重复流程；语音则由本地 Kokoro/MeloTTS 生成并统一响度、编码与验收：
...
完整约束见 [多语言、玩家档案与全量配音](docs/08-多语言玩家档案与全量配音.md)。
